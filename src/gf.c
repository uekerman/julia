...
jl_code_instance_t *jl_compile_method_internal(jl_method_instance_t *mi, size_t world)
{
    ...
    if (mi2 != mi) {
        ...
        if (jl_atomic_load_relaxed(&codeinst->invoke) == NULL) {
            ...
            specsig.bits = jl_atomic_load_acquire(&codeinst2->specsigflags);
            ...
            if (unspecmi) {
                ...
                if (unspec && (unspec_invoke = jl_atomic_load_acquire(&unspec->invoke))) {
                    ...
                    if (unspec_fptr) {
                        ...
                        _jl_specsig_flags_t specsig;
                        specsig.bits = jl_atomic_load_acquire(&unspec->specsigflags);
                        while (!specsig.flags.specptr_matches_invokeptr) {
                            jl_cpu_pause();
                            specsig.bits = jl_atomic_load_acquire(&unspec->specsigflags);
                        }
    ...
    if (!codeinst) {
        ...
        if (unspec_fptr) {
            // wait until invoke and specsigflags are properly set
            _jl_specsig_flags_t specsig;
            specsig.bits = jl_atomic_load_acquire(&ucache->specsigflags);
            while (!specsig.flags.specptr_matches_invokeptr) {
                 jl_cpu_pause();
                 specsig.bits = jl_atomic_load_acquire(&ucache->specsigflags);
            }
